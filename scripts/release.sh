...
# 2. Create a release branch and commit version bump changes
# 3. Auto-generate CHANGELOG from git commits (requires manual review!)
# 4. Open a pull request
# 5. Create the tag automatically after the PR is merged
#
...
# Release changes must enter protected main through a pull request.
if ! git diff-index --quiet HEAD --; then
    echo -e "${RED}❌ Error: Working directory must be clean before starting a release${NC}"
    git status --short
    exit 1
fi

echo -e "${BLUE}🔄 Syncing main with origin...${NC}"
git pull --ff-only origin main

if git show-ref --verify --quiet "refs/heads/$RELEASE_BRANCH"; then
    echo -e "${RED}❌ Error: Local branch $RELEASE_BRANCH already exists${NC}"
    exit 1
fi

if git ls-remote --exit-code --heads origin "$RELEASE_BRANCH" >/dev/null 2>&1; then
    echo -e "${RED}❌ Error: Remote branch $RELEASE_BRANCH already exists${NC}"
    exit 1
fi
...
if ! git diff-index --quiet HEAD --; then
    ...
    git add package.json package-lock.json src/config/app-config.ts README.md CHANGELOG.md
...
echo "  1. ✅ Push branch: $RELEASE_BRANCH"
echo "  2. ✅ Open release PR into main"
echo "  3. ✅ Create tag $VERSION after the PR is merged"
echo "  4. ✅ Trigger GitHub Release and Vercel deployment"
echo ""
...
if command -v gh >/dev/null 2>&1; then
    gh pr create \
        ...
        --body "Release $VERSION: version files and changelog generated by scripts/release.sh."
else
    echo -e "${YELLOW}⚠️ GitHub CLI not found. Create a PR manually:${NC}"
    echo "https://github.com/$GITHUB_REPO/compare/main...$RELEASE_BRANCH?expand=1"
fi
echo ""

# Final success
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✨ Release PR $VERSION successfully created!${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo -e "${CYAN}🔗 Resources:${NC}"
echo "  📊 Build Status: https://github.com/$GITHUB_REPO/actions"
echo "  🔀 Release PR: https://github.com/$GITHUB_REPO/compare/main...$RELEASE_BRANCH?expand=1"
echo "  🌐 Live Demo: https://nawaetu.com"
echo ""
echo -e "${CYAN}⏳ Deployment Pipeline:${NC}"
echo "  1. GitHub Actions validates the release PR"
echo "  2. Merge the PR after all required checks pass"
echo "  3. GitHub Actions creates tag $VERSION automatically"
echo "  4. GitHub Release and Vercel production deployment follow"
echo ""
