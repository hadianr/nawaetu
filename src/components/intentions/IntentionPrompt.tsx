...
export default function IntentionPrompt({
    ...
    const { greetingText, greetingEmoji } = (() => {
        if (hour >= 3 && hour < 11) return { greetingText: t.niat_morning_title, greetingEmoji: "🌅" };
        if (hour >= 11 && hour < 15) return { greetingText: t.niat_afternoon_title, greetingEmoji: "☀️" };
        if (hour >= 15 && hour < 18) return { greetingText: t.niat_evening_title, greetingEmoji: "🌤️" };
        return { greetingText: t.niat_night_title, greetingEmoji: "🌙" };
    })();
    ...
    const content = (
        ...
        >
            ...
            >
                ...
                {/* Header */}
                <div className={`relative p-6 border-b ${isDaylight ? "border-slate-100" : "border-white/5"}`}>
                    <div className="text-center relative z-10 space-y-4">
                        <div className="text-4xl drop-shadow-lg mb-2">{greetingEmoji}</div>
                        <h2 className={`text-2xl font-serif font-bold mb-1 ${isDaylight ? "text-slate-900" : "text-white"}`}>{greetingText}</h2>
                        ...
                        <AnimatePresence mode="wait">
                            {dalil && (
                                ...
                                >
                                    {dalil.arabic && (
                                        <p className={`text-lg md:text-xl font-arabic leading-loose ${isDaylight ? "text-slate-800" : "text-slate-200"}`} dir="rtl">
                                            {dalil.arabic}
                                        </p>
                                    )}
                                    <p className={`text-xs md:text-sm font-serif italic leading-relaxed px-4 ${isDaylight ? "text-slate-600" : "text-slate-400"}`}>
                                        "{isId ? dalil.textId : dalil.textEn}"
                                    </p>
                                    <p className="text-[10px] font-medium text-[rgb(var(--color-primary))] opacity-80 tracking-widest uppercase">
                ...
                {/* Content */}
                <div className={`p-6 space-y-5 relative z-10 ${isDaylight ? "bg-slate-50/50" : "bg-transparent"}`}>
                    {/* Textarea */}
                    <div className="relative group">
                        ...
                        <textarea
                            ...
                            placeholder={t.niat_placeholder || "Bismillah, niat saya hari ini adalah..."}
                            maxLength={500}
                            rows={4}
                            className={`w-full bg-transparent border-0 border-b-2 px-2 py-4 focus:outline-none resize-none transition-all font-serif text-lg leading-relaxed shadow-none ${isDaylight ? "border-slate-200 text-slate-900 placeholder:text-slate-400 focus:border-emerald-500/50" : "border-white/10 text-white placeholder:text-white/30 focus:border-[rgb(var(--color-primary))]/50"}`}
                            style={{
                    ...
                    <div className="pt-2">
                        <div className="flex items-center justify-between mb-3 px-1">
                            <label className={`text-[10px] font-bold uppercase tracking-[0.2em] ${isDaylight ? "text-slate-400" : "text-white/50"}`}>{t.niat_need_inspiration}</label>
                            <button
                                onClick={() => {
                    ...
                    <button
                        ...
                        className={`w-full py-4 rounded-xl border disabled:opacity-50 disabled:cursor-not-allowed font-serif font-bold shadow-lg transition-all transform active:scale-[0.98] text-lg mt-2 ${isDaylight ? "bg-slate-900 border-slate-900 text-white hover:bg-slate-800" : "bg-[rgb(var(--color-primary))] border-[rgb(var(--color-primary))] text-white hover:opacity-90"}`}
                    >
                        {isSubmitting ? (
                            <div className="flex items-center justify-center gap-3 text-white">
                                <Loader2 className="w-5 h-5 animate-spin" />
                                <span>{t.niat_save_niat_btn}</span>
                            </div>
                        ) : (
                            <span className="text-white">{t.niat_set_intention_btn}</span>
                        )}
                    </button>
                </div>
            </motion.div>
