...
export default function MissionListModal({
    ...
    const tabs: TabDefinition[] = useMemo(() => [
        { id: 'all', label: t.missionTabAll || "All", icon: "target", activeColorClass: "" },
        { id: 'obligatory', label: t.missionTabObligatory || "Obligatory", icon: "star", activeColorClass: "" },
        { id: 'sunnah_prayer', label: t.missionTabSunnahPrayer || "Sunnah Prayer", icon: "landmark", activeColorClass: "" },
        { id: 'dhikr', label: t.missionTabDhikr || "Dhikr", icon: "hands", activeColorClass: "" },
        { id: 'fasting', label: t.missionTabFasting || "Fasting", icon: "moon", activeColorClass: "" },
        { id: 'quran', label: t.missionTabQuran || "Quran", icon: "book", activeColorClass: "" },
        { id: 'recommended', label: t.missionTabRecommended || "Recommended", icon: "sparkles", activeColorClass: "" }
    ], [t]);
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                isCompleted
                                    ...
                                    : "bg-[rgb(var(--color-surface))] border-[rgb(var(--color-border))] hover:bg-[rgb(var(--color-primary))]/5 hover:border-[rgb(var(--color-primary))]/30"
                            )}
                        >
                            ...
                            <span className="p-2 rounded-xl bg-[rgb(var(--color-surface-subtle))]">
                                <AppIcon name={mission.iconKey ?? resolveAppIconName(mission.icon)} size="lg" tone={isCompleted ? "muted" : "primary"} />
                            </span>
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-1">
                                    <p className={cn(
                                        ...
                                        isCompleted ? "text-[rgb(var(--color-text-muted))] line-through" : isSpecial ? "text-[rgb(var(--color-accent))]" : "text-[rgb(var(--color-text-strong))]"
                                    )}>
                                        {mission.title}
                                    </p>
                                ...
                                <p className="text-xs truncate mb-1 text-[rgb(var(--color-text-muted))]">{mission.description}</p>

                                <div className="flex items-center gap-2 mt-1 flex-wrap">
                                    <span className="text-[10px] font-mono text-[rgb(var(--color-accent))]">+{mission.hasanahReward} Hasanah</span>
                                        ...
                                        return (
                                            <span
                                                ...
                                                className={cn(
                                                    ...
                                                    ref
                                                        ? "bg-[rgb(var(--color-primary))]/10 text-[rgb(var(--color-primary-strong))] border-[rgb(var(--color-primary))]/20 hover:bg-[rgb(var(--color-primary))]/20 hover:underline"
                                            ...
                                            >
                                                <span className="flex items-center gap-1"><AppIcon name="book" size="xs" tone="muted" /> {mission.dalil}</span>
                                    ...
                                    {isLocked && (
                                        <span className="text-[9px] flex items-center gap-0.5 px-1.5 py-0.5 rounded border text-[rgb(var(--color-text-muted))] bg-[rgb(var(--color-surface-subtle))] border-[rgb(var(--color-border))]">
                                            {t.home_mission_locked}
                                        </span>
                                    )}
                                    {!isLocked && validation.isLate && (
                                        <span className="text-[9px] text-[rgb(var(--color-danger))] flex items-center gap-1 font-bold bg-[rgb(var(--color-danger))]/10 px-1.5 py-0.5 rounded border border-[rgb(var(--color-danger))]/20">
                                            <AlertCircle className="w-3 h-3" /> {t.home_mission_late}
                                        </span>
                                    )}
                                    {!isLocked && validation.isEarly && (
                                        <span className="text-[9px] text-[rgb(var(--color-primary-strong))] flex items-center gap-1 font-bold bg-[rgb(var(--color-primary))]/10 px-1.5 py-0.5 rounded border border-[rgb(var(--color-primary))]/20">
                            ...
                            <div className={cn(
                                ...
                                isCompleted
                                    ...
                                    : "border-2 border-[rgb(var(--color-border))] group-hover:border-[rgb(var(--color-primary))]"
                            )}>
    ...
    return (
        <Dialog open={finalOpen} onOpenChange={finalOnOpenChange}>
            ...
            <DialogContent
                ...
                className="mission-list-modal w-[95%] max-w-md h-auto max-h-[85vh] p-0 gap-0 overflow-hidden rounded-[32px] flex flex-col bg-[rgb(var(--color-surface))] border border-[rgb(var(--color-border))] text-[rgb(var(--color-text-strong))] shadow-[var(--shadow-floating)] backdrop-blur-xl"
                onOpenAutoFocus={() => {
                    if (initialTab) setActiveTab(initialTab);
                }}
            >
                <DialogHeader className="p-5 pb-3 border-b border-[rgb(var(--color-border))] bg-[rgb(var(--color-surface-subtle))] relative">
                    <button
                        onClick={() => finalOnOpenChange?.(false)}
                        className="absolute right-4 top-4 w-8 h-8 flex items-center justify-center rounded-full transition-colors z-20 bg-[rgb(var(--color-primary))]/10 hover:bg-[rgb(var(--color-primary))]/20"
                    >
                ...
                <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="w-full h-full flex flex-col overflow-hidden">
                            ...
                            {tabs.map(tab => (
                                <TabsTrigger
                                    ...
                                    className={cn(
                                        ...
                                        "border-[rgb(var(--color-border))] bg-[rgb(var(--color-surface))] text-[rgb(var(--color-text-muted))] hover:bg-[rgb(var(--color-primary))]/10 data-[state=active]:bg-[rgb(var(--color-primary))] data-[state=active]:text-[rgb(var(--color-primary-foreground))] data-[state=active]:border-[rgb(var(--color-primary))] shadow-[var(--shadow-card)]"
                                    )}
                                >
                                    <AppIcon name={tab.icon} size="xs" tone={activeTab === tab.id ? "default" : "muted"} />
                                    {tab.label}
                                </TabsTrigger>
                            ))}
                        </TabsList>
                    ...
                    <div className="px-4 pt-3 pb-5 flex-1 overflow-hidden bg-[rgb(var(--color-surface))]">
                        {tabs.map(tab => (
